...
export class Robot {
    ...
    */
     ...
     * @param text : Text to look for on the button or link
     * @description Find button or a element that content text contact */
    public  find(text:string): Promise<any>{
        return new Promise(async(resolve) => {
            const elt = await this.current_page.$eval('button, a',(element:any, search_text:string)=>{
                if(element.textContent.toLowerCase().includes(search_text)) {
